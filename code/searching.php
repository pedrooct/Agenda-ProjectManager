...
{
	$searchIn= $_GET['search'];
}
else {
	header('location: dashboard?search=error');
}

$data= $ask->searchbar($searchIn,$isProfessor);
$getimg= new Ask();
...
<body>
	<div class="header">
		<div class="home-menu pure-menu pure-menu-horizontal pure-menu-fixed">
			...
			<a class="pure-menu-heading" href="dashboard">Clique para voltar </a>
			<a href="dashboard">
...
<div class="content">
	<div class="pure-g">
		<?php
		if(strcmp($data[0],"utilizador")==0 && !empty($data[1] && !empty($data[1][0])))
		{
						...
						<ul class="price">
							<li class="headerBox">
								...
								if($row->{'foto'}==null)
								{
									...
									echo '<img id="uploadPreview" class="imgpreview" src="data:'.$row->{'foto_tipo'}.';base64, '.$row->{'foto'}.'" alt="imagem perfil" style="width: 120px; height: 120px;" />';
								}
								?>
							</li>
							<li class="grey"> <?php echo $row->{'user_name'};?> </li>
							<li class="grey"> <?php echo "Email UFP: ".$row->{'email'};?> </li>
							<?php
							if ($row->{'tipo'}==1) {
								if(!empty($row->{'email_pessoal'}))
								{
									?>
									<li class="grey"> <?php echo "Email Pessoal: ".$row->{'email_pessoal'};?> </li>
									<?php
								}
							}
							if($row->{'tipo'}==1)
							{
								if($row->{'formacao_avancada'}==0)
								{
									echo '<li class="grey"> Formação: Mestrado </li>';
								}
								else if($row->{'formacao_avancada'}==1)
								{
									echo '<li class="grey"> Formação: Doutoramento </li>';
								}
								else if($row->{'formacao_avancada'}==2)
								{
									echo '<li class="grey">  Formação: Pós-Douturamentos </li>';
								}
							}
							?>
							<li class="grey"><?php echo "Contacto: ".$row->{'contacto'}; ?></li>
							<?php if(!empty($row->{'skype_id'}))
							{?>
								<li class="grey"><?php echo "SkypeID: "."@".$row->{'skype_id'}; ?></li>
								<?php
							}
							if(!empty($row->{'cacifo_digital_id'})){
								?>
								<li class="grey">
									<a href= <?php echo $row->{'cacifo_digital_id'}; ?>>elearningURL</a>
								</li>
								<?php
							}
						}
			...
			<?php
			if(strcmp($data[2],"evento")==0)
			{
				for($j=0;$j< sizeof($data[3]);$j++)
				{
					$row=$data[3][$j];
					$user=$ask->getUserInfoID($row->{'convidado_id'});
					$project=$ask->getProjectBasedID($row->{'project_id'});
					?>
					<div class=".pure-u-1-1 pure-u-sm-1-2 pure-u-md-1-2 pure-u-lg-1-4">
						<div class="columns">
							<ul class="price">
								...
								<?php if($isProfessor && $perfil_id==$user["perfil_id"])
								{
									?>
									<li class="grey-opt">
										<a href=<?php echo "../code/scripts/deleteEvent?proid=".$row->{'id'}."&PID=".$userSender['perfil_id'] ;?> name="no" class="pure-button-no">Rejeitar</a>
									</li>
									<?php
								}
								if($isProfessor && $perfil_id==$userSender["perfil_id"])
								{
									?>
									<li class="grey-opt">
										<a href=<?php echo "../code/scripts/deleteEvent?proid=".$row['id']."&PID=".$user['perfil_id'] ;?> name="no" class="pure-button-no">Rejeitar</a>
									</li>
									<?php
								}
								?>
			...
			?>
			<?php
			if(strcmp($data[4],"email")==0 && !empty($data[5]))
			{
				for($j=0;$j< sizeof($data[5]);$j++)
				{
					$row=$data[5][$j];
					?>
					<div class=".pure-u-1-1 pure-u-sm-1-2 pure-u-md-1-2 pure-u-lg-1-4">
		...
		if($isProfessor)
		{
			...
			<div id="add_icon" class="dropdownadd-contentadd">
				...
				<a href="estatisticas"> Ver estatistica</a>
			</div>
			<?php
		}
		else {
