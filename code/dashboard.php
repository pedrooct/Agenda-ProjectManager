...
$eventsPS= $ask->getEventsPendingSent($perfil_id);
$eventsPR= $ask->getEventsPendingRecieved($perfil_id);
...
<div class="content">
	<p>Pedidos para Reunião</p>
	<div class="pure-g">
		<?php
		while($row = mysqli_fetch_array($eventsPS))
		{
			$user=$ask->getUserInfoID($row['convidado_id']);
			$userSender=$ask->getUserInfoID($row['utilizador_id']);
			...
			<div class=".pure-u-1-1 pure-u-sm-1-2 pure-u-md-1-2 pure-u-lg-1-4">
				<div class="columns">
					<ul class="price">
						...
						<?php
						if(!$ask->checkIsUserPerfilId($row['utilizador_id'],$perfil_id))
						{
							?>
							<li class="grey"> <?php echo "Enviado de ". $userSender['user_name'] ;?> </li>
							<?php
						}
						else {
							?>
							<li class="grey"> <?php echo "Enviado a  ".$user['user_name'] ;?> </li>
							<?php
						}
						?>
						...
						<li class="grey"> Á espera de confirmação... </li>
					</ul>
				</div>
			</div>
			<?php
		}
		while ($row = mysqli_fetch_array($eventsPR)) {
			$user=$ask->getUserInfoID($row['convidado_id']);
			$userSender=$ask->getUserInfoID($row['utilizador_id']);
			$project=$ask->getProjectBasedID($row['project_id']);
			?>
			<div class=".pure-u-1-1 pure-u-sm-1-2 pure-u-md-1-2 pure-u-lg-1-4">
				<div class="columns">
					<ul class="price">
						...
						<li class="grey"> <?php echo "Hora: ".$row['hora_inicio']."-".$row['hora_fim'] ;?> </li>
						<li class="grey"> <?php echo $project['nome']."-".$row['nome'] ;?> </li>
						<li class="grey"> <?php echo  $userSender['email'] ;?> </li>
						<li class="grey"><?php echo "Conctato:".$row['contacto'] ;  ?></li>
						<li class="grey"> <?php echo "Sala: ".$row['sala'] ;?> </li>
						<?php
						if(!$ask->checkIsUserPerfilId($row['utilizador_id'],$perfil_id))
						{
							...
							<li class="grey"> <?php echo "Enviado de ". $userSender['user_name'] ;?> </li>
							<li class="grey-opt">
								<a href=<?php echo "../code/scripts/acceptEvent?proid=".$row['id']."&PID=".$userSender['perfil_id'] ;?> name="yes"class="pure-button-ok">Aceitar</a>
							</li>
							<li class="grey-opt">
							...
							?>
							<li class="grey"> <?php echo "A pedido de ". $name ;?> </li>
							<?php
						}
						?>
						<li class="grey"><?php echo "Notas: ".$row['notas'] ;  ?></li>
