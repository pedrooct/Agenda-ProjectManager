...
<body>
	...
	{
		if(!empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['contacto']) && !empty($_POST['cpassword']))
		{
			$ask = new Ask();
			...
			if($response)
			{
				if(strcmp($semail,$_POST['email'])==0)
				{
					...
					if ($response)
					{
						...
						$response = $get->updateInfoProf($user_name,$email,$password,$foto,$foto_ex,$contacto,$skype_id,$cacifo_id,$notificacao);
						if($response)
						{
	...
	<div class="pure-g">
		<div class="l-box-lrg pure-u-2 pure-u-md-2-5">
			<form class="pure-form pure-form-stacked" enctype="multipart/form-data" method="POST">
				...
				<input id="upload" name="foto" class="file-upload__input" type="file" value= <?php echo $data['foto']?> onchange="PreviewImage();">
